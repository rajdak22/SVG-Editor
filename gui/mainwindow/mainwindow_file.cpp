...
void MainWindow::saveFile() {
  if (file_path_.isEmpty())
    ...
    std::ofstream file(
        file_path_.toStdString());  // Opening file stream using the file_path_
...
void MainWindow::saveFileAs() {
  ...
  QString fileName =
      QFileDialog::getSaveFileName(this, header, defaultDir, fileType);
...
void MainWindow::openFile() {
  ...
  QString fileType = "SVG Files (*.svg)";
  QString fileName =
      QFileDialog::getOpenFileName(this, header, defaultDir, fileType);
  if (fileName.isEmpty()) return;

  file_path_ = fileName;

  Diagram d = SVGParser::parseFile(
      fileName.toStdString());  // parsing file to get Diagram objects
...
void MainWindow::closeFile() {
  close();  // QT function that handles memory deletion, and other processes on
            // closing.
}
