...
Diagram SVGParser::parseFile(const std::string& filename) {
  ...
  while (std::getline(file, line)) {
    ...
    if (obj != nullptr) diagram.addObject(obj);
  }
...
std::string SVGParser::getAttribute(const std::string& line,
...
void SVGParser::setPaintAttributes(const std::string& line,
                                   const std::shared_ptr<GraphicsObject>& obj) {
  ...
  if (!strokeWidth.empty()) obj->setStrokeWidth(std::stoi(strokeWidth));
}
